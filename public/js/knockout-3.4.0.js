...
ko.bindingHandlers.autocomplete = {    
    init: function(element, valueAccessor, allBindings, viewModel, bindingContext) {
        ...
        if('autocompleteoptions' in allBindings()) {
            var options = allBindings().autocompleteoptions;
            ...
            $(element).autocomplete(options)
                    ...
                    ._renderItem = function( ul, item ) {
                                        return $( "<li>" )
                                            .append( "<a class=\"autocomplete-text\">" + item.label + "</a>" )
                                            .appendTo( ul );
                                    };
        }
    },
...
ko.bindingHandlers.numerico = {
    init: function(element, valueAccessor, allBindingsAccessor) {
        if ($(element).is(':input'))
        {
            $(element).maskMoney({thousands:'', decimal:',',precision:'0', mask:'99999999,99', allowZero: true, defaultZero: true});
...
(function(){
    var format = function(value) {        
        ...
        var jaAdicionouCasas = false
        var cont = 0;

        return $.map(toks.split('').reverse(), function(elm, i) {

                if (elm != "\.") {
                        if (!jaAdicionouCasas && cont % 2 === 0 && cont > 0) {
                                jaAdicionouCasas = true;				
                                cont = 0;
                                return [',', elm];				
                        }

                        cont++;					
                        return [(cont % 3 === 0 && cont > 0 ? '\.' : ''), elm];
                }
    ...
    // example
    //self.cash = ko.observable('150.00').money();
    ko.subscribable.fn.money = function() {
        ...
        var writeTarget = function(value) {
            ...
            target(stripped.replace(/(\d)(\d{2})$/,"$1.$2"));
        };
        ...
        result.formatted = ko.computed({
            read: function() {
                return format(target() ? target().toString().replace("\.", "") : "");
            },
            write: writeTarget
        });
    ...
    init: function(element, valueAccessor, allBindingsAccessor, context) {
        ...
        $(element).sortable({
            update: function(event, ui) {
                ...
                if(options.ordenar != undefined && typeof(options.ordenar) == 'function'){
                    options.ordenar();
                }
            }
        });
